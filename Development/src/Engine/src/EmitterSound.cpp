...
namespace vega
{
	EmitterSound::EmitterSound(const std::string& name, const std::string& fileName, bool loop, bool stream){
		sound = GetEngine()->GetAudio()->createSound(name, fileName, loop, stream);
	}
