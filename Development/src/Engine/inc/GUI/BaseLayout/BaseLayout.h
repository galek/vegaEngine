	...
	protected:
		...
		void assignWidget(T * & _widget, const std::string& _name, bool _throw = true, bool _createFakeWidgets = true)
		{
			...
			for (MyGUI::VectorWidgetPtr::iterator iter = mListWindowRoot.begin(); iter != mListWindowRoot.end(); ++iter)
			{
				...
				if (nullptr != find)
				{
					...
					{
						Warning("[GUI]Widget with name '%s' have wrong type ('%s')instead of '%s'). [%s]", _name.c_str(),
							find->getTypeName().c_str(), T::getClassTypeName().c_str(), mLayoutName.c_str());
			...
			Warning("[GUI]Widget with name '%s' in '%s'  not found. ", _name.c_str(), mLayoutName.c_str());
			if (_createFakeWidgets)
				_widget = _createFakeWidget<T>(mMainWidget);
		}
		...
		void initialise(const std::string& _layout, MyGUI::Widget* _parent = nullptr, bool _throw = true, bool _createFakeWidgets = true)
		{
			...
			else
			{
				...
				{
					Warning("[GUI]Root widget with name '%s' or '%s'  not found. [%s]", MAIN_WINDOW1.c_str(), MAIN_WINDOW2.c_str(), mLayoutName.c_str());
					if (_createFakeWidgets)
