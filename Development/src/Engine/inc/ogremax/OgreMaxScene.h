...
#include "OgreMaxRenderWindowIterator.h"


//Classes----------------------------------------------------------------------
namespace vega
{
    class LevelLoaderCallback;
     ...
     * Ogre scene manager. As such, when deleting an LevelLoader, keep in mind that
     ...
     */
    class LevelLoader : public Ogre::RenderTargetListener
    {
    public:
        LevelLoader();
        virtual ~LevelLoader();
        ...
        enum
        {
            ...
            /**
             * By default, the LevelLoader::Load() checks to see if the file that was passed
        ...
        void Load
            ...
            LevelLoaderCallback* callback = 0,
			const Ogre::String& defaultResourceGroupName = Ogre::ResourceGroupManager::AUTODETECT_RESOURCE_GROUP_NAME
            );
        ...
        /**
         * Destroys the internal LevelLoader objects
    ...
    protected:
         ...
         * This will be destroyed when the LevelLoader is destroyed, so if you
         ...
         * that are contained in the LevelLoader
         */
    ...
    {
    public:
        virtual ~LevelLoaderCallback() {}

        /** Called before a scene is to be loaded. */
        virtual void StartedLoad(const LevelLoader* scene) {}

        virtual void CreatedSceneManager(const LevelLoader* scene, Ogre::SceneManager* sceneManager) {}

        virtual void CreatedExternal(const LevelLoader* scene, const Types::ExternalItem& item) {}
        virtual void CreatedExternalUserData(const LevelLoader* scene, const Types::ExternalUserData& userData) {}

        //Called when scene (not per-object) user data is loaded. Will only be called if there is user data
        virtual void LoadedUserData(const LevelLoader* scene, const Ogre::String& userDataReference, const Ogre::String& userData) {}

        //Called when resource locations are loaded. Will only be called if there are resource locations
        virtual void LoadingResourceLocations(const LevelLoader* scene) {}
        virtual void LoadedResourceLocations(const LevelLoader* scene, const LevelLoader::ResourceLocations& resourceLocations) {}
        ...
        virtual void LoadingRenderTexture(const LevelLoader* scene, Types::RenderTextureParameters& renderTextureParameters) {}
        virtual void LoadingSceneFile(const LevelLoader* scene, const Ogre::String& fileName, Ogre::String& resourceGroupName) {}
        virtual void LoadingSkyBox(const LevelLoader* scene, Types::SkyBoxParameters& skyBoxParameters) {}
        virtual void LoadingSkyDome(const LevelLoader* scene, Types::SkyDomeParameters& skyDomeParameters) {}
        virtual void LoadingSkyPlane(const LevelLoader* scene, Types::SkyPlaneParameters& skyPlaneParameters) {}
        virtual void LoadingEntity(const LevelLoader* scene, Types::EntityParameters& parameters) {}
        virtual void LoadingPlane(const LevelLoader* scene, Types::PlaneParameters& parameters) {}
         ...
         */
        virtual void LoadingNodeAnimation(const LevelLoader* scene, Types::NodeAnimationParameters& parameters) {}
        ...
        virtual void CreatedLight(const LevelLoader* scene, Ogre::Light* light) {CreatedMovableObject(scene, light);}
        virtual void CreatedCamera(const LevelLoader* scene, Ogre::Camera* camera) {CreatedMovableObject(scene, camera);}
        virtual void CreatedMesh(const LevelLoader* scene, Ogre::Mesh* mesh) {}
        virtual void CreatedEntity(const LevelLoader* scene, Ogre::Entity* entity) {CreatedMovableObject(scene, entity);}
        virtual void CreatedParticleSystem(const LevelLoader* scene, Ogre::ParticleSystem* particleSystem) {CreatedMovableObject(scene, particleSystem);}
        virtual void CreatedBillboardSet(const LevelLoader* scene, Ogre::BillboardSet* billboardSet) {CreatedMovableObject(scene, billboardSet);}
        virtual void CreatedPlane(const LevelLoader* scene, const Ogre::Plane& plane, Ogre::Entity* entity) {CreatedMovableObject(scene, entity);}
        virtual void CreatedMovableObject(const LevelLoader* scene, Ogre::MovableObject* object) {}

        virtual void CreatedNodeAnimation(const LevelLoader* scene, Ogre::SceneNode* Node, Ogre::Animation* animation) {}
        virtual void CreatedNodeAnimationTrack(const LevelLoader* scene, Ogre::SceneNode* node, Ogre::AnimationTrack* animationTrack, bool enabled, bool looping) {}
        virtual void CreatedNodeAnimationState(const LevelLoader* scene, Ogre::SceneNode* node, Ogre::AnimationState* animationState) {}

        //Render texture creation callbacks
        virtual Ogre::Camera* GetRenderTextureCamera(const LevelLoader* scene, const Types::RenderTextureParameters& renderTextureParameters) {return 0;}

        virtual void CreatedRenderTexture(const LevelLoader* scene, const Types::LoadedRenderTexture* renderTexture) {}

        /** Called after a node is created, but before its entities or any child nodes have been created */
        virtual void StartedCreatingNode(const LevelLoader* scene, Ogre::SceneNode* node) {}

        /** Called after the node and all its entities and child nodes have been created */
        virtual void FinishedCreatingNode(const LevelLoader* scene, Ogre::SceneNode* node) {}

        virtual void CreatedTerrain(const LevelLoader* scene, const Ogre::String& dataFile) {}

        virtual void HandleObjectExtraData(Types::ObjectExtraDataPtr objectExtraData) {}

        //Progress callback
        virtual void UpdatedLoadProgress(const LevelLoader* scene, Ogre::Real progress) {}
         ...
         */
        virtual void FinishedLoad(const LevelLoader* scene, bool success) {}
         ...
         */
        virtual void CreatingShadowTextures(const LevelLoader* scene, Types::ShadowParameters& shadowParameters) {}
    };
