...
namespace vega
{
	...
	TerrainMaterialGeneratorD::TerrainMaterialGeneratorD()
	{
		...
		mProfiles.push_back(new SM2Profile(this, "SM2", "Profile for rendering on Shader Model 2 capable cards"));
	...
	//---------------------------------------------------------------------
	//---------------------------------------------------------------------
	TerrainMaterialGeneratorD::SM2Profile::SM2Profile(TerrainMaterialGenerator* parent, const std::string& name, const std::string& desc)
	...
	//---------------------------------------------------------------------
	Ogre::uint8 TerrainMaterialGeneratorD::SM2Profile::getMaxLayers(const Terrain* terrain) const
	{
	...
	MaterialPtr TerrainMaterialGeneratorD::SM2Profile::generate(const Terrain* terrain)
	{
		...
		if (mat.isNull())
		{
			...
			if (mat.isNull())
			{
				mat = matMgr.create(matName, "Shaders");
			}
		}
		// clear everything
	...
	MaterialPtr TerrainMaterialGeneratorD::SM2Profile::generateForCompositeMap(const Terrain* terrain)
	{
		...
		if (mat.isNull())
		{
			...
			// it's important that the names are deterministic for a given terrain, so
			// use the terrain pointer as an ID
			const std::string& matName = terrain->getMaterialName() + "/comp";
			mat = matMgr.getByName(matName);
			if (mat.isNull())
			{
				mat = matMgr.create(matName, "Shaders");
	...
	//---------------------------------------------------------------------
	std::string TerrainMaterialGeneratorD::SM2Profile::ShaderHelper::getChannel(uint idx)
	{
		uint rem = idx % 4;
		switch(rem)
		{
	...
	//---------------------------------------------------------------------
	std::string TerrainMaterialGeneratorD::SM2Profile::ShaderHelper::getVertexProgramName(
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		std::string progName = terrain->getMaterialName() + "/sm2/vp";
	...
	//---------------------------------------------------------------------
	std::string TerrainMaterialGeneratorD::SM2Profile::ShaderHelper::getFragmentProgramName(
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
 
		std::string progName = terrain->getMaterialName() + "/sm2/fp";
		...
		const SM2Profile* prof, const Terrain* terrain, TechniqueType tt)
	{
		...
		std::string progName = getVertexProgramName(prof, terrain, tt);
		HighLevelGpuProgramPtr ret = mgr.getByName(progName);
		if (ret.isNull())
		{
			ret = mgr.createProgram(progName, "Shaders", 
	...
	{
		HighLevelGpuProgramManager& mgr = HighLevelGpuProgramManager::getSingleton();
		std::string progName = getFragmentProgramName(prof, terrain, tt);
	...
	{
		uint uvIdx = layer / 2;
		std::string uvChannels = (layer % 2) ? ".zw" : ".xy";
		uint blendIdx = (layer-1) / 4;
		std::string blendChannel = getChannel(layer-1);
		std::string blendWeightStr = std::string("blendTexVal") + Ogre::StringConverter::toString(blendIdx) + 
			"." + blendChannel;
