...
namespace vega
{
	...
	bool TextureManager::_updateNormalMap(Image &Image)
	{
		if (!mCreated)
		{
			Debug("Error in TextureManager::_updateNormalMap, create() does not called.");
		...
		{
			Debug("Error in TextureManager::_updateNormalMap, Image type isn't correct.");
		...
		{
			Debug(("Error in TextureManager::update, Update size doesn't correspond to " + getTextureName(TEX_NORMAL_ID) + " texture size").c_str());
