...
namespace vega
{
	...
	{
		Debug("[Hydrax]GodRaysManager::_createMaterials");
		...
		GRD_Technique0_Pass0->setSceneBlending(Ogre::SBT_TRANSPARENT_ALPHA/*Nick*/);//NickGalko
	...
	void GodRaysManager::addDepthTechnique(Ogre::Technique *Technique, const bool& AutoUpdate)
	{
		if (!Ogre::MaterialManager::getSingleton().resourceExists(_def_GodRaysDepth_Material_Name))
		{
			Debug("GodRaysManager::addDepthTechnique(...) Objects intersection must be enabled and create() already called, skipping...");
