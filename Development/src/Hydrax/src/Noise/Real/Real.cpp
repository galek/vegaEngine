...
namespace vega{ namespace Noise{
...
bool Real::eraseWave(int id)
{
    if( (id < 0) || (id >= (int)mWaves.size()) ){
        Debug("Error (Real::eraseWave):\tCan't delete the wave.");
        Debug("\tIdentifier exceed the waves vector dimensions.");
        return false;
    }
    ...
    if(mWaves.size() == Size){
        Debug("Error (Real::eraseWave):\tCan't delete the wave.");
        Debug("\tThe size before deletion matchs with the size after the operation.");
        return false;
    }
    return true;
}
