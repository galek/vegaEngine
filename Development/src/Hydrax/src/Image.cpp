...
namespace vega
{
	Image::Image(const Size &_Size)
		: mSize(Size(_Size.Width + 1, _Size.Height + 1))
	...
	Image::Image(const Size &_Size, const Type &Type)
		: mSize(Size(_Size.Width + 1, _Size.Height + 1))
	...
	Image::Image(const Size &_Size, const Type &Type, const float &v)
		: mSize(Size(_Size.Width + 1, _Size.Height + 1))
		, mChannels(static_cast<int>(Type))
		, mData(0)
	{
	...
	const float& Image::getValue(const int &x, const int &y, const int &c) const
	{
		...
		if (c < 0 || c > mChannels ||
			...
			y < 0 || y > mSize.Height)
		{
			Debug("Error in Image::getValue, x = " + Ogre::StringConverter::toString(x) 
    ...
    Image::Pixel Image::getPixel(const int &x, const int &y) const
	{
		...
		if (x < 0 || x > mSize.Width ||
			y < 0 || y > mSize.Height)
		{
			Debug("Error in Image::getPixel, x = " + Ogre::StringConverter::toString(x) 
				                             + " y = " + Ogre::StringConverter::toString(y));
	...
	void Image::setValue(const int &x, const int &y, const int &c, const float &v)
	{
		...
		if (c < 0 || c > mChannels   ||
			...
			y < 0 || y > mSize.Height)
		{
			Debug("Error in Image::setValue, x = " + Ogre::StringConverter::toString(x) 
				                             + " y = " + Ogre::StringConverter::toString(y) 
											 + " Channel = " + Ogre::StringConverter::toString(c));
	...
	void Image::setPixel(const int &x, const int &y, const Pixel &p)
	{
		...
		if (x < 0 || x > mSize.Width ||
			y < 0 || y > mSize.Height)
		{
			Debug("Error in Image::setPixel, x = " + Ogre::StringConverter::toString(x) 
				                             + " y = " + Ogre::StringConverter::toString(y));
