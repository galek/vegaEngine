...
#include "libs/DetourTileCache/DetourTileCache.h"
#include <float.h>
...
namespace vega
{
	...
	OgreDetourTileCache::OgreDetourTileCache(AI *recast, int tileSize)
		: m_recast(recast),
	...
	{
		float position[3];
		AI::OgreVect3ToFloatA(pos, position);
		int tx; int ty;
	...
	dtObstacleRef OgreDetourTileCache::removeTempObstacle(Ogre::Vector3 raySource, Ogre::Vector3 rayHit)
	{
		...
		AI::OgreVect3ToFloatA(raySource, sp);
		AI::OgreVect3ToFloatA(rayHit, sq);
	...
	void OgreDetourTileCache::unloadTiles(const Ogre::AxisAlignedBox &areaToUpdate)
	{
		...
		float bmin[3], bmax[3];
		AI::OgreVect3ToFloatA(areaToUpdate.getMinimum(), bmin);
		AI::OgreVect3ToFloatA(areaToUpdate.getMaximum(), bmax);
		dtCompressedTileRef touched[DT_MAX_TOUCHED_TILES];
		int ntouched = 0;
	...
	std::vector<dtCompressedTileRef> OgreDetourTileCache::getTilesContainingBox(Ogre::Vector3 boxMin, Ogre::Vector3 boxMax)
	{
		...
		// calculate bounds
		float bmin[3]; float bmax[3];
		AI::OgreVect3ToFloatA(boxMin, bmin);
		AI::OgreVect3ToFloatA(boxMax, bmax);
