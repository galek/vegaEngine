...
namespace vega
{
		...
		tris(0)
	{
		...
		bmin = new float[3];
		bmax = new float[3];
		AI::OgreVect3ToFloatA(tileBounds.getMinimum(), bmin);
		AI::OgreVect3ToFloatA(tileBounds.getMaximum(), bmax);
	...
	void InputGeom::calculateExtents()
	{
		...
		if (!bmax)
			bmax = new float[3];
		AI::OgreVect3ToFloatA(min, bmin);
		AI::OgreVect3ToFloatA(max, bmax);
	}
	...
	Ogre::AxisAlignedBox InputGeom::getBoundingBox()
	{
		...
		Ogre::Vector3 max;
		Ogre::Vector3 min;
		AI::FloatAToOgreVect3(bmin, min);
		AI::FloatAToOgreVect3(bmax, max);
		bb.setMaximum(max);
		bb.setMinimum(min);
