...
#include "libs/DetourTileCache/DetourTileCacheBuilder.h"
...
namespace vega
{
	bool AI::STATIC_GEOM_DEBUG = false;
	bool AI::VERBOSE = true;

	AI::AI(Ogre::SceneManager* sceneMgr, OgreRecastConfigParams configParams)
	...
	void AI::configure(OgreRecastConfigParams params)
	{
	 ...
	 **/
	bool AI::NavMeshBuild(std::vector<Ogre::Entity*> srcMeshes)
	{
		if (srcMeshes.empty()) {
	...
	bool AI::NavMeshBuild(InputGeom* input)
	{
	 ...
	 **/
	int AI::FindPath(float* pStartPos, float* pEndPos, int nPathSlot, int nTarget)
	{
		dtStatus status;
		dtPolyRef StartPoly;
	...
	int AI::FindPath(Ogre::Vector3 startPos, Ogre::Vector3 endPos, int nPathSlot, int nTarget)
	{
		float start[3];
		float end[3];
	...
	std::vector<Ogre::Vector3> AI::getPath(int pathSlot)
	{
	...
	void AI::drawPolyMesh(const struct rcPolyMesh &mesh, bool colorRegions)
	{
	...
	void AI::CreateRecastPolyMesh(const Ogre::String name, const unsigned short *verts, const int nverts, const unsigned short *polys, const int npolys, const unsigned char *areas, const int maxpolys, const unsigned short *regions, const int nvp, const float cs, const float ch, const float *orig, bool colorRegions)
	{
		m_flDataX = npolys;
		m_flDataY = nverts;
	...
	float AI::getAgentRadius()
	{
		return m_agentRadius;
	}

	float AI::getAgentHeight()
	{
		return m_agentHeight;
	}

	float AI::getPathOffsetFromGround()
	{
		return m_pathOffsetFromGround;
	}

	float AI::getNavmeshOffsetFromGround()
	{
		return m_navMeshOffsetFromGround;
	}

	rcConfig AI::getConfig()
	{
		return m_cfg;
	}
	...
	std::vector<Ogre::Vector3> AI::getManualObjectVertices(Ogre::ManualObject *manual)
	{
	...
	void AI::CreateRecastPathLine(int nPathSlot)
	{
	...
	void AI::OgreVect3ToFloatA(const Ogre::Vector3 vect, float* result)
	{
		result[0] = vect[0];
		result[1] = vect[1];
		result[2] = vect[2];
	...
	void AI::FloatAToOgreVect3(const float* vect, Ogre::Vector3 &result)
	{
		result.x = vect[0];
		result.y = vect[1];
		result.z = vect[2];
	...
	Ogre::Vector3 AI::getRandomNavMeshPoint()
	{
		float resultPoint[3];
	...
	void AI::setPointExtents(Ogre::Vector3 extents)
	{
		OgreVect3ToFloatA(extents, mExtents);
	}

	Ogre::Vector3 AI::getRandomNavMeshPointInCircle(Ogre::Vector3 center, Ogre::Real radius)
	{
	...
	Ogre::String AI::getPathFindErrorMsg(int errorCode)
	{
	...
	bool AI::findNearestPointOnNavmesh(Ogre::Vector3 position, Ogre::Vector3 &resultPt)
	{
		dtPolyRef navmeshPoly;
		return findNearestPolyOnNavmesh(position, resultPt, navmeshPoly);
	}

	bool AI::findNearestPolyOnNavmesh(Ogre::Vector3 position, Ogre::Vector3 &resultPt, dtPolyRef &resultPoly)
	{
		float pt[3];
	...
	void AI::removeDrawnNavmesh(unsigned int tileRef)
	{
	...
	OgreRecastNavmeshPruner* AI::getNavmeshPruner()
	{
