...
namespace vega
{
	OgreDetourCrowd::OgreDetourCrowd(AI *recast)
		: m_crowd(0),
		m_recast(recast),
		m_targetRef(0),
		m_activeAgents(0)
	...
	int OgreDetourCrowd::addAgent(const Ogre::Vector3 position)
	{
		...
		float p[3];
		AI::OgreVect3ToFloatA(position, p);
	...
	{
		float pos[3];
		AI::OgreVect3ToFloatA(position, pos);

		float tgt[3];
		AI::OgreVect3ToFloatA(target, tgt);

		float res[3];
		calcVel(res, pos, tgt, speed);

		Ogre::Vector3 result;
		AI::FloatAToOgreVect3(res, result);
	...
	{
		Ogre::Vector3 result;
		AI::FloatAToOgreVect3(m_targetPos, result);
		return result;
	}
	...
	bool OgreDetourCrowd::requestVelocity(int agentId, Ogre::Vector3 velocity)
	{
		...
		float vel[3];
		AI::OgreVect3ToFloatA(velocity, vel);
