...
namespace vega
{
	/** SkyX class
		...
		@remarks How to update SkyX:
				 ...
				 important. 
				 The per-frame update must be performed at first: vega::update(...), after that and before each
				 camera render operation vega::notifyCameraRender(...) must be invoked.
     ...
     */
	class DllExport SkyX : public Ogre::RenderTargetListener
	{
	public:
		/** Render queue groups
		 */
		struct RenderQueueGroups
		{
			 ...
			 */
			FORCEINLINE RenderQueueGroups(const Ogre::uint8& s, const Ogre::uint8& vc, const Ogre::uint8& vclu, const Ogre::uint8& vclo)
		 ...
		 */
		SkyX(Ogre::SceneManager* sm, Ogre::Camera* _cam,
        ...
        /** Update (to be invoked per frame)
            ...
            @remarks Invoke this function only one time per frame.
					 Per-frame update must be performed before per-camera updates through vega::notifyCameraRender(...)
		...
		/** Notify camera render (to be invoked per camera and per frame)
			...
			@remarks Invoke this method manually before the camera render operation
				     Per-camera updates must be performed after the per-frame update through vega::update(...)
			         Also it's possible to use listeners, making all this process transparent,
		 ...
		 */
		FORCEINLINE void setTimeMultiplier(const Ogre::Real& TimeMultiplier)
		{
		 ...
		 */
		FORCEINLINE const Ogre::Real& getTimeMultiplier() const
		{
		 ...
		 */
		FORCEINLINE MeshManager* getMeshManager()
		{
			return mMeshManager;
		}
		 ...
		 */
		FORCEINLINE AtmosphereManager* getAtmosphereManager()
		{
		...
		/** Get GPU manager
		    @return Atmosphere manager pointer
		 */
		FORCEINLINE GPUManager* getGPUManager()
		{
			return mGPUManager;
		}
		...
		/** Get moon manager
		    @return Moon manager
		 */
		FORCEINLINE MoonManager* getMoonManager()
		{
			return mMoonManager;
		}
		 ...
		 */
		FORCEINLINE CloudsManager* getCloudsManager()
		{
		 ...
		 */
		FORCEINLINE VCloudsManager* getVCloudsManager()
		{
		...
		/** Set controller
		    @param c Controller
		 */
		FORCEINLINE void setController(Controller* c)
		{
		 ...
		 */
		FORCEINLINE const LightingMode& getLightingMode() const
		{
		 ...
		 */
		FORCEINLINE const bool& isStarfieldEnabled() const
		{
			return mStarfield;
		}
		 ...
		 */
		FORCEINLINE void setInfiniteCameraFarClipDistance(const Ogre::Real& d)
		{
		 ...
		 */
		FORCEINLINE const Ogre::Real& getInfiniteCameraFarClipDistance() const
		{
		 ...
		 */
		FORCEINLINE const Ogre::Real& _getTimeOffset() const
		{
			return mTimeOffset;
		}
