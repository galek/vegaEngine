...
namespace vega
{
	...
	SoundManager::SoundManager(const std::string& deviceName, int maxNumSources) :
		...
		mLastDeltaTime(0.0f)
	{
		Ogre::LogManager::getSingleton().logMessage("*-*-* vega Initialization");
	...
	{
		Ogre::LogManager::getSingleton().logMessage("*-*-* vega Shutdown");
	...
	iSound* SoundManager::createSound(const std::string& name,
		const std::string& fileName, bool loop, bool stream)
	{
		// Lock Mutex
	...
	bool SoundManager::Update(float _time)
	{
	...
	ALboolean SoundManager::eaxSetBufferMode(ALsizei numBuffers, BufferRef *buffers, EAXMode bufferMode)
	{
		// Lock Mutex
		OGREAL_LOCK_AUTO_MUTEX
	...
	void SoundManager::initializeDevice(const std::string& deviceName)
	{
		...
		*/
		...
		Ogre::LogManager::getSingleton().logMessage("Choosing: " + std::string(alcGetString(mDevice, ALC_DEVICE_SPECIFIER)));
