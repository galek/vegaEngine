...
*/
size_t AudioALOggStreamRead(void *ptr, size_t size, size_t nmemb, void *datasource)
{
	Ogre::DataStreamPtr dataStream = *reinterpret_cast<Ogre::DataStreamPtr*>(datasource);
	return dataStream->read(ptr, size);
}

int AudioALOggStreamSeek(void *datasource, ogg_int64_t offset, int whence)
{
	Ogre::DataStreamPtr dataStream = *reinterpret_cast<Ogre::DataStreamPtr*>(datasource);
	switch(whence)
	{
...
int AudioALOggStreamClose(void *datasource)
{
	return 0;
}

long AudioALOggStreamTell(void *datasource)
{
	Ogre::DataStreamPtr dataStream = *reinterpret_cast<Ogre::DataStreamPtr*>(datasource);
...
namespace vega {
	OggSound::OggSound(SoundManager*_mSoundManager,const std::string& name, const Ogre::DataStreamPtr& soundStream, bool loop, bool stream) :
		Sound(_mSoundManager,name, soundStream->getName(), stream)
	{
		try
		{
			...
			callbacks.close_func = AudioALOggStreamClose;
			callbacks.tell_func = AudioALOggStreamTell;
			callbacks.read_func = AudioALOggStreamRead;
			callbacks.seek_func = AudioALOggStreamSeek;
	...
	void OggSound::setSecondOffset(float seconds)
	{
		if(seconds < 0) return;
	...
	float OggSound::getSecondOffset()
	{
		if(!mStream)
		{
		...
		else
		{
			...
			*/
			...
			float oggStreamOffset = ov_time_tell(&mOggStream);
			float bufferOffset = Sound::getSecondOffset();

			float totalOffset = oggStreamOffset - (0.25 + (0.25 - bufferOffset));
			return totalOffset;
		}
	}
