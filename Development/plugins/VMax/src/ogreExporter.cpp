	...
	**/
	INT_PTR CALLBACK IGameExporterOptionsDlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_INITDIALOG:
		{
							  ...
							  //Versioning
							  TCHAR Title[256];
							  _stprintf(Title, _T("VMax version %.1f"), EXVERSION);
...
#endif
		std::string xmlConfPath = plugConfDir + "\\VMax\\config.xml";
	...
	void OgreSceneExporter::loadExportConf(std::string path, ParamList &param)
	{
		...
		if (xmlDoc.LoadFile(path.c_str()))
		{
			...
			TiXmlElement*child = rootElem->FirstChildElement("IDC_RESPREFIX");
			if (child)
	...
	void OgreExporter::saveExportConf(std::string path)
	{
		...
		TiXmlElement*child = new TiXmlElement("IDC_RESPREFIX");
		TiXmlText*childText = new TiXmlText(m_params.resPrefix.c_str());
	...
	bool OgreExporter::exportScene()
	{
		...
		logFileName.append("\\export.log");
		VMax::VMaxLogFile::SetPath(logFileName.data());
		...
		Ogre::LogManager::getSingleton().createLog("Render.log", true);
		...
		_mkdir((std::string(plugConfDir + "\\VMax")).c_str());

		std::string xmlIGameConfPath = plugConfDir + "\\VMax\\IGameProp.xml";
		...
		std::string sTitle = "VMax: " + m_params.sceneFilename;
#ifdef UNICODE
		std::wstring title_w;
	...
	bool OgreExporter::exportNode(IGameNode* pGameNode, TiXmlElement* parent)
	{
		...
		if (IsNodeToExport(pGameNode))
		{
			...
			VMaxLog("Found node: %ls\n", nodeName.data());
#else
			VMaxLog("Found node: %s\n", pGameNode->GetName());
#endif
...
#ifdef UNICODE
														MSTR nodeName = pGameNode->GetName();
														VMaxLog("Found mesh node: %ls\n", nodeName.data());
#else
														VMaxLog("Found mesh node: %s\n", pGameNode->GetName());
#endif
...
#ifdef UNICODE
														 MSTR nodeName = pGameNode->GetName();
														 VMaxLog("Found light: %ls\n", nodeName.data());
#else
														 VMaxLog("Found light: %s\n", pGameNode->GetName());
#endif
														 if (sceneData)
														 {
...
#ifdef UNICODE
														  MSTR nodeName = pGameNode->GetName();
														  VMaxLog("Found camera: %ls\n", nodeName.data());
#else
														  VMaxLog("Found camera: %s\n", pGameNode->GetName());
#endif
														  if (sceneData)
														  {
...
class VMaxClassDesc :public ClassDesc2
{
public:
	int 			IsPublic() { return TRUE; }
	void *			Create(BOOL loading = FALSE) { return new VMax::OgreSceneExporter(); }
	const TCHAR *	ClassName() { return _T("VMax"); }
	SClass_ID		SuperClassID() { return SCENE_EXPORT_CLASS_ID; }
	Class_ID		ClassID() { return Class_ID(0x6a042a9d, 0x75b54fc4); }
	const TCHAR* 	Category() { return _T("Vega Engine Tools"); }
	const TCHAR*	InternalName() { return _T("VMax"); }
	HINSTANCE		HInstance() { return hInstance; }
};
static VMaxClassDesc VMaxDesc;
ClassDesc2* GetVMaxDesc() { return &VMaxDesc; }