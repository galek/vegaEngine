...
namespace EasyOgreExporter
{
  ...
  bool ExSkeleton::getVertexBoneWeights(IGameMesh* pGameMesh)
  {
    ...
    //force the last key
		...
		a.m_tracks.clear();
    a.m_length = (float(length) / float(rate)) / GetFrameRate();
      ...
      //load a keyframe for every joint at current time
        ...
        //set key time
        key.time = (float((times[i] - times[0])) / float(rate)) / GetFrameRate();
	...
	// Get animations
	std::vector<Animation>& ExSkeleton::getAnimations()
	{
		return m_animations;
	}
