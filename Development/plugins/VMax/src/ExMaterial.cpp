...
#include "../nvidia-texture-tools/src/nvimage/DirectDrawSurface.h"
...
namespace VMax
{
	...
	{
#ifdef UNICODE
		VMaxLog("Debug: Material properties : %ls\n", prop->GetName());
#else
		VMaxLog("Debug: Material properties : %s\n", prop->GetName());
#endif
		...
		if (pUVGen)
		{
			...
			if (prop)
			{
				...
				if (fabs(rotU) > PRECISION)
					VMaxLog("Warning: U rotation detected. This is unsupported.\n");
			}
			prop = pUVGen->GetVAngleData();
			{
				prop->GetPropertyValue(rotV);
				if (fabs(rotV) > PRECISION)
					VMaxLog("Warning: V rotation detected. This is unsupported.\n");
			}
				...
				if (prop->IsPBlock2())
				{
					...
					if (pTexmap)
					{
...
#endif
						VMaxLog("Info : Texture class name: %s.\n", pName);
						...
						else if (!strcmp(pName, "Bitmap"))
						{
							...
							VMaxLog("Texture Name: %ls\n", texName.data());
#else
							std::string texName = pBitmapTex->GetMapName();
							VMaxLog("Texture Name: %s\n", texName.c_str());
#endif
							...
							{
								VMaxLog("Updated texture location: %s.\n", absPath.c_str());
							}
							else
							{
#ifdef UNICODE
								VMaxLog("Warning: Couldn't locate texture: %ls.\n", texName.data());
#else
								VMaxLog("Warning: Couldn't locate texture: %s.\n", texName.c_str());
#endif
							}
	...
	void ExMaterial::loadStandardMaterial(IGameMaterial* pGameMaterial)
	{
		...
		for (int i = 0; i < texCount; ++i)
		{
			...
			VMaxLog("Info : texture properties %s\n", pPropName.c_str());
			}
			*/

#ifdef UNICODE
			VMaxLog("Exporting %d texture from %ls...\n", i, texClass.data());
			if (pGameTexture && (pGameTexture->IsEntitySupported() || (texClass == L"Normal Bump")) || (texClass == L"Vertex Color"))
#else
			VMaxLog("Exporting %d texture from %s...\n", i, texClass.c_str());
...
#endif
			{
				...
				case ID_AM:
				{
							  VMaxLog("Ambient channel texture.\n");
							  if (m_ambientLocked)
							  {
								  VMaxLog("Ambient channel locked, we use the diffuse texture instead.\n");
				...
				case ID_DI:
				{
							  VMaxLog("Diffuse channel texture.\n");
							  tex.bCreateTextureUnit = bFoundTexture;
					...
					break;
				case ID_SP:
					VMaxLog("Specular channel texture.\n");
					tex.bCreateTextureUnit = bFoundTexture;
					m_hasSpecularMap = true;
					tex.type = ID_SP;
					break;
				case ID_SH:
					VMaxLog("SH channel texture.\n");
					tex.type = ID_SH;
					break;
				case ID_SS:
					VMaxLog("Shininess Strenth channel texture.\n");
					tex.type = ID_SS;
					break;
				case ID_SI:
					VMaxLog("Self-illumination channel texture.\n");
					tex.bCreateTextureUnit = bFoundTexture;
					tex.type = ID_SI;
					break;
				case ID_OP:
					VMaxLog("opacity channel texture.\n");
					tex.bCreateTextureUnit = bFoundTexture;
					...
					break;
				case ID_FI:
					VMaxLog("Filter Color channel texture.\n");
					tex.type = ID_FI;
					break;
				case ID_BU:
					VMaxLog("Bump channel texture.\n");
					tex.bCreateTextureUnit = bFoundTexture;
					...
					break;
				case ID_RL:
					VMaxLog("Reflection channel texture.\n");
					tex.type = ID_RL;
					tex.bCreateTextureUnit = bFoundTexture;
					...
					break;
				case ID_RR:
					VMaxLog("Refraction channel texture.\n");
					tex.type = ID_RR;
					break;
				case ID_DP:
					VMaxLog("Displacement channel texture.\n");
					tex.type = ID_DP;
					break;
				}
	...
	bool ExMaterial::load()
	{
		...
		if (m_GameMaterial)
		{
			...
			if (!pGameMaterial->IsEntitySupported())
			{
				...
				{
					VMaxLog("Warning: Non supported material : %s... enumerate properties\n", matClass.c_str());
